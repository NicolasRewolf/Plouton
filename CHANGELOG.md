...
**Trouvé, non corrigé** — les âges du simulateur de prestation compensatoire ne
sont pas assainis (`prestation-compensatoire.ts:86`), contrairement aux revenus
et aux années. Un champ vide donne « NaN € » ; un `null` est lu comme « zéro an »
et **rabote 20 % en silence**. La valeur par défaut est un arbitrage juridique :
laissé à Nicolas, épinglé tel quel par `check:baremes`.
